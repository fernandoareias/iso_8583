...
mark_as_repeat(Message) ->
    ...
    M4Updated = case M4 of
        $0 -> $1;  % Request -> Repeat request
        $1 -> $1;  % Already repeat request
        $2 -> $3;  % Response -> Repeat response
        $3 -> $3;  % Already repeat response
        $4 -> $5;  % Advice -> Repeat advice
        $5 -> $5;  % Already repeat advice
        _ -> M4
    end,
        ...
        true ->
            ...
            case byte_size(Mti) of
                4 ->
                    ...
                    <<M1, M2, M3, M4>> = Mti,
                    case is_digit(M1) andalso is_digit(M2) andalso
                         is_digit(M3) andalso is_digit(M4) of
                        true -> ok;
                        false -> {error, invalid_mti}
                    end;
                _ ->
                    {error, invalid_mti}
            end;
...
convert_to_response_mti(Message) ->
    ...
    M3Response = case M3 of
        $0 -> $1;  % Request -> Response request
        $1 -> $1;  % Already response request
        $2 -> $3;  % Response -> Response response
        $3 -> $3;  % Already response response
        $4 -> $5;  % Advice -> Response advice
        $5 -> $5;  % Already response advice
        _ -> M3
    end,
