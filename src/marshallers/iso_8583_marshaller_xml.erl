-module(iso_8583_marshaller_xml).
...
%% Exported Functions
%%
-export([marshal/1, unmarshal/1, marshal_field/3, unmarshal_field/3, marshal_end/2,
         unmarshal_end/2, marshal_init/1, unmarshal_init/2, marshal_bitmap/1, unmarshal_bitmap/1,
         marshal_mti/1, unmarshal_mti/1]).
...
-spec marshal(iso8583message()) -> string().
marshal(Message) ->
    iso_8583_marshaller:marshal(Message, ?MARSHALLER_XML).
...
unmarshal(Marshalled) ->
    iso_8583_marshaller:unmarshal(Marshalled, ?MARSHALLER_XML).
...
marshal_field(FieldId, FieldValue, _EncodingRules) ->
    ...
    MarshalledContent =
        iso_8583_marshaller:marshal(FieldValue,
                                    [{field_marshaller, ?MODULE}, {mti_marshaller, ?MODULE}]),
    "<isomsg id=\""
    ++ Id
    ++ "\""
...
marshal_end(Message, Marshalled) ->
    ...
    "<isomsg" ++ encode_attributes(Attrs) ++ ">\n" ++ Marshalled ++ "</isomsg>\n".
...
unmarshal_complex([Field | T], Iso8583Msg) when is_record(Field, xmlElement) ->
    ...
    UpdatedMsg = iso_8583:set(FieldId, Value, Iso8583Msg),
