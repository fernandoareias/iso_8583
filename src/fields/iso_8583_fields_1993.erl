...
get_field_name(?DATE_EFF) ->
    <<"Date, Effective">>;
get_field_name(?POS_DATA_CODE) ->
    <<"POS Data Code">>;
get_field_name(?FUNCTION_CODE) ->
    <<"Function Code">>;
get_field_name(?MESSAGE_REASON_CODE) ->
    <<"Message Reason Code">>;
get_field_name(?CARD_ACCEPTOR_BUSINESS_CODE) ->
    <<"Card Acceptor Business Code">>;
get_field_name(?AMOUNT_ORIGINAL) ->
    <<"Amount, Original">>;
get_field_name(?ACQUIRER_REFERENCE_DATA) ->
    <<"Acquirer Reference Data">>;
get_field_name(?ICC_SYSTEM_RELATED_DATA) ->
    <<"ICC System Related Data">>;
get_field_name(?ORIGINAL_DATA_ELEMENTS_1993) ->
    <<"Original Data Elements">>;
get_field_name(?BITMAP_TERTIARY) ->
    <<"Bitmap, Tertiary">>;
